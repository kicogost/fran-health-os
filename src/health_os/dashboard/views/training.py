...
if not daily_load_series:
    ...
    st.subheader("Monotony / strain (trailing 7 days)")
    ...
    if mono["confidence"] == "insufficient_data":
        ...
        st.write(
            f"Weekly load: {mono['weekly_load']:.0f}. Monotony undefined (zero variance this week)."
        )
    else:
    ...
    st.subheader("Load by day and sport")
    if not activities.empty:
        ...
        if not by_sport.empty:
            ...
            fig2.update_layout(barmode="stack", title="Activity training_load by sport")
            st.plotly_chart(fig2, width="stretch")
        else:
            st.write("No activities have a `training_load` value yet to break down by sport.")

st.divider()
st.subheader("Calisthenics progression")
st.info(

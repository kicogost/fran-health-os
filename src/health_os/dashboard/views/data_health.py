...
for col, (field, label) in zip(cols, FRESHNESS_COLUMNS.items(), strict=True):
    ...
    col.metric(
        label, f"{days_stale}d ago" if days_stale > 0 else "today", help=f"last: {last_date}"
    )

st.subheader("Missing days (trailing 30)")
if not daily.empty:
...
if ingest_runs.empty:
    ...
    display["errors"] = display["errors"].apply(lambda e: "" if e in (None, "[]", "null") else e)

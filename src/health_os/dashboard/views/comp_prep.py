...
if trend["confidence"] == "insufficient_data":
    st.caption("Trend slope: insufficient data (needs 3+ real weigh-ins in the trailing 21 days).")
else:
...
if trend["confidence"] != "insufficient_data":
    ...
    fig.add_trace(
        go.Scatter(
            x=projection_dates, y=proj_high, mode="lines", line=dict(width=0), showlegend=False
        )
    )
    fig.add_trace(
        go.Scatter(
...
fig.update_layout(title="Weight trajectory vs. required path")
